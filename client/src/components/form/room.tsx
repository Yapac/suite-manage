...
import { Room } from "@/utils/schemas.types";

type Props = {
  initialValues: {
    roomId?: { label: string; value: string }[];
  };
  cancelForm: () => void;
};

export const RoomForm = ({ initialValues, cancelForm }: Props) => {
  ...
  const { selectProps } = useSelect<Room>({
    ...
    optionLabel: (item) => `Room ${item.number} - ${item.type}`,
  });
